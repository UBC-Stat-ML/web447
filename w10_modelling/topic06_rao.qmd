...
- In languages that do not support discrete latent variables ([for example, Stan](https://mc-stan.org/docs/stan-users-guide/latent-discrete.html)), 
  this is the only way to implement certain models (e.g. mixture models, coming next)
...
```stan {shortcodes=true, filename="chernobyl_rao_blackwellized.stan"}
{{< include chernobyl_rao_blackwellized.stan >}}
```
...
```{r}
fit$summary(NULL, c("mean", "median", "sd", "mad", "rhat", "ess_bulk"))
```
...
Chain 1 finished in 1.6 seconds.
  ...
  ...
    mean                  3.41    2.77  2.61  1.37   1.00   37126  ...
```
