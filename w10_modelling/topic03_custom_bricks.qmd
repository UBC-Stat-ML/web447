...
- Understanding why Stan represents the density in log-scale. 
...
- Applying this to our Kumaraswamy example
    - Use: $\log(f(x)) = \log(a) + \log(b) + (a-1) \log(x) + (b-1) \log(1 - x^a).$
...
2. Use the special suffix `_lpdf` for your function name, which stands for "log probability density function."
3. Then you can use the distribution as usual. From this it is apparent that Stan's `~` notation is just a shortcut for `target += distributionName_lpdf(...)`.
