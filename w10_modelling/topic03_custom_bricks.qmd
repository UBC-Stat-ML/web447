...
Even though Stan has a long list of built-in distributions, you will eventually run into a situation where you 
...
- In the `model` block:
    - Use `target += [logarithm (base e) of the density]`
...
```stan {shortcodes=true, filename="kumaraswamy.stan"}
{{< include kumaraswamy.stan >}}
```
1. Support is enforced. 
2. Contribution of the Kumaraswamy log density added to the log joint density.

```{r message=FALSE, warning=FALSE, results=FALSE}
mod = cmdstan_model("kumaraswamy.stan")
fit = mod$sample(
  seed = 1,
  refresh = 0,
  data = list(a = 2, b = 5),  
...
mcmc_hist(fit$draws("x")) + theme_minimal()
```
...
**Key fact:** Stan, like most numerical methods, uses [double-precision floating point](https://en.wikipedia.org/wiki/Floating-point_arithmetic) approximation of real numbers. 

- Suppose you have 500 observations.
- Each has a likelihood of about $1/10$. 
...
**Question:** Why does R return zero?

::: {.callout-caution collapse="true"} 
## Click for answer
- Because $(1/10)^{500}$ is smaller than the smallest positive number representable in double precision ($\approx 10^{-300}$)
...
```stan {shortcodes=true, filename="kumaraswamy_as_function.stan"}
{{< include kumaraswamy_as_function.stan >}}
```
...
3. Then you can use your distribution name using `~` as usual (notice we do not include the `_lpdf` when we make such call). From this it is apparent that Stan's `~` notation is just a shortcut for `target += distributionName_lpdf(...)`. 

```{r message=FALSE, warning=FALSE, results=FALSE}
mod = cmdstan_model("kumaraswamy_as_function.stan")
fit = mod$sample(
  seed = 1,
  refresh = 0,
  data = list(a = 2, b = 5),  
