...
- They evaluated the performance of the Bayes estimator as follows::
    - apply the estimators: $a^\parm = \best(Y^\parm)$, 
    - compute the losses $l^\parm = L(a^\parm, X^\parm)$, and 
    - return the average loss as the measure of performance: $\frac{1}{M} \sum l^\parm$.
...
so that we can rewrite the left-hand side of @eq-opt as follows:
