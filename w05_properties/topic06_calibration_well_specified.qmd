...
The question we consider now: **are [credible interval](../w02_discrete_bayes/topic05_credible.qmd) 
calibrated?**
...
- This leads to frequentist analysis of Bayesian procedures---a very useful thing to do!

**Calibration of credible intervals**: 90% credible interval is calibrated if it contains the true parameter **90% of the time**.

**What do we mean by "true parameter"?** 

- Start with a joint distribution, $p^\star(x, y)$, which we will call "nature."
- For each "experiment": use $p^\star$ to simulate both a true parameter, $x^\star$, and an associated dataset $y^\star$: $(x^\star, y^\star) \sim p^\star$. ($=$ notion of [forward simulation](../w01_discrete_inference/topic06_forward_sampling.qmd)).

**What do we mean by "90% credible interval"?** a function $C(y) = [L(y), R(y)]$ which (1) computes the posterior $\pi(\cdot | y)$, and (2), selects left and right end points, $l$, $r$ such that $\int_l^r \pi(x | y) \dee x = 0.9$. 
...
we will now run the algorithm described above under "What do we mean by "90% of the time"?"...
