...
&=  \argmin \{ \Pr[X \le c|Y] + 1 - \Pr[X \le d |Y] + k(d - c) : [c,d] \in A \} \\
&=  \argmin \{ \Pr[X \le c|Y] - \Pr[X \le d |Y] + k(d - c) : [c,d] \in A \}
\end{aligned} 
$$

Here we assume the posterior has a continuous density $f$ to change $<$ into $\le$. We have also used that removing "$+1$" does not change the value of the $\argmin$. 
