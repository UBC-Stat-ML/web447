...
This week, we will give a new interpretation to the "bag of coins" example 
...
we will use the exact same probability model as the bag of coins example from last week. 

Use the following correspondence to link last week's example with this week's:

- $(Y_i = 1)$: 
    - $\leftrightarrow$ "$i$-th flip is a tails" 
    - $\leftrightarrow$ "$i$-th launch is a success"
- $(X = k)$:
    - $\leftrightarrow$ "the coin drawn from bag has probability $p = k/K$ of tails"
...
- In the bag of coins example, we can replicate the whole "experiment" several times (both prior and likelihood).
