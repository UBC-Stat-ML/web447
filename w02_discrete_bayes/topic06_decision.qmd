...
Compute $\min L(a)$ and $\argmin L(a)$. 
...
Next, let us simplify the expected loss function:
$$\ex[L(a, X)] = \ex[\ind[a = 0] \cdot (\$0) + \ind[a = 1, X = 0] \cdot (\$9) +  \ind[a = 1, X = 1] \cdot (-\$10)].$$

Next, use **linearity of expectation** which you will recall from probability theory tells us that 
...
**Mathematically:** use an action $a \in A$ minimizing
$$\argmin \ex[L(a, X) | Y = y].$$
where:
