...
Some knowledge of how MCMC works is useful to identify when it fails ("MCMC diagnostics"). 
...
- The lab will show this can be relaxed.[^1] 

[^1]: This relaxation yields a generalization of the algorithm covered in the lecture today.
    ...
    - Sample an **acceptance Bernoulli**:
      $$A^\parm \sim \distBern(\min(1, R^\parm)).$$
        - If $A^\parm = 1$, we **accept** the proposed sample:
          $$X^\parm = \tilde X^\parm,$$
        - Else, $A^\parm = 0$, and we **reject** the proposed sample and stay at the previous position:
          $$X^\parm = X^{(m-1)}.$$ 
...
- When proposing to a point with **higher** posterior density, we have:
    - $\gamma(\tilde X^\parm) > \gamma(X^{(m-1)})$
    - hence $R^\parm > 1$, 
    - so $\min(1, R^\parm) = 1$, 
    - and $A^\parm$ is always one, i.e. always accepted!
- When proposing to a point with **lower** posterior density, we have:
    - $\gamma(\tilde X^\parm) < \gamma(X^{(m-1)})$
    - hence $R^\parm < 1$, 
    - so $\min(1, R^\parm) = R^\parm$,
    - $A^\parm \sim \distBern(\gamma(\tilde X^\parm)/\gamma(X^{(m-1)}))$,
    - i.e., acceptance probability is a ratio of target densities at two points (proposed vs previous). 


## Visualization 

In this week's exercise, you will implement MH with a standard normal proposal. 
...
**Terminology:** the above plot (samples as a function of MCMC iteration) is called a **trace plot.**
