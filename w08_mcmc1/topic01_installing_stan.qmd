...
[^4]: Older version of these instructions were using `RStan` instead of `cmdstanr`. Unfortunately, 
      the former seems to have abruptly stopped working (at least on my machine) circa MacOS 15.7.4, and 
      development seems to have migrated from  `RStan` into `cmdstanr`. As a result, we are currently 
