...
**Look at the Figures:** Consider the following situations, where let us say in all case you observe that the **planes are always full**. 

a. Pairs of cities, where each pair of cities is assigned a fixed number of planes going back and forth between the two cities. 
b. A **tour** where a fixed number of planes, equal to the number of cities, go around the world on a common route. 
c. A central "hub", where all flights land at the hub. 

**Follow-up:** how to express "fraction of planes between cities $x$ and $x'$" in terms of $\mu_i$, $K$? 
