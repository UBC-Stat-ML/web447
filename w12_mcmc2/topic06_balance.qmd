...
a. Pairs of cities, where each pair of cities is assigned a fixed number of planes going back and forth between the two cities (planes always full). 
b. A **tour** where a fixed number of planes go around the world on a common route (again, planes always full). 
c. A central "hub", where all flights either land or take off from the hub (again, planes always full). 
...
to get an equation characterizing $\pi$-invarinace in terms of $K$. 
...
**Question:** what is the relationship between local and global balance?
