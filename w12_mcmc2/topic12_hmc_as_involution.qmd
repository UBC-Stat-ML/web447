...
- How to build an [involution](topic11_involution.qmd) for ["rolling a ball for time $t$ starting at current location and momentum"](topic10_hmc_intuition.qmd)?
...
- As often, it is easier to describe how **things change** rather than how things are,
...
- we typically do not have closed-form solution for the ODE of motion
...
**Key idea:** update only one of $\{x, p\}$ at a time to get [triangular Jacobian matrices](topic11_involution.qmd#examples-of-involution) (easier to analyze). 
...
- Do so in a palindromic fashion to obtain an involution after flipping.
...
We can now define $\Kh$: it is the [deterministic proposal MH](topic11_involution.qmd) with proposal $T = \Tf \circ \Tlf^L$. 
