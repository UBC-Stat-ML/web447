...
It is easy for bugs to sneak into MCMC code. 
...
However, the test discussed there still left "MCMC too slow" as one possible issue confounding a software defect. 

Here we describe a new test (the *exact invariance test*) that excludes "MCMC too slow" from the possible causes of warning. 
When the *exact invariance test* raises a warning, the only possible causes are "software defects" and "bad luck."
...
Can we automatically detect bugs in MCMC code?
...
- Call our forward simulator. 
  $$(x, y) \sim \text{forward()}. $$
- Run an MCMC algorithm **initialized** with the $(x, y)$ we just forward simulated. 
...
- Typically, we do MCMC on $x$ only...
    - ...but here, view the MCMC as a Markov chain on **pairs** $(x, y)$.
...
- How to check this?
    - Let's use frequentist tools!
        - Here `ks.test`, the Kolmogorov-Smirnov test, but a two-samples, unpaired `t.test` would have worked too. 
