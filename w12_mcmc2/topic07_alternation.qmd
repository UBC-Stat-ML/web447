...
1. Sample from $K_1$ based on the current state.
2. Sample from $K_2$ based the state output by $K_1$. 


### Coding up kernel alternation

Here is an example of how to implement the alternation.
...
```
1. Sample from $K_1$ based on the input state.
2. Sample from $K_2$ based on the state output by $K_1$.

### Mathematical notation for kernel alternations

**Question:** write a mathematical expression for the alternation kernel $\Kalt$ in terms of $K_1$ and $K_2$. 
...
**Proposition:** if each $K_i$ is $\pi$-invariant, then their alternation is $\pi$-invariant. 
