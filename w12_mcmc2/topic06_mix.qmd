...
This page shows one way we can establish invariance of the overall algorithm "for free" 
...
- Idea of **kernel mixture**: at each MCMC iteration, suppose we start at $x$
    ...
    - if tails, use $K_2$, i.e., $x' \sim K_2(\cdot | x)$.
...
Based on these kernels, 
here is an example of how to implement the mixture:
...
**Proposition:** if each $K_i$ is $\pi$-invariant, then their mixture is $\pi$-invariant. 

**Question:** can you prove this result? Convince yourself at the same time that the argument crucially depends on $\rho_i$
