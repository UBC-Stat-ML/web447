...
(namely, [as we saw](topic10_hmc_intuition.qmd), the proposal is to "let the marble roll a little bit").

We state a version of the MH algorithm based on such a deterministic proposal here.  
...
**Calculus review:** do you remember what the Jacobian determinant is?
...
\nabla T(x) &= \begin{bmatrix}
    \dfrac{\partial T_1}{\partial x_1} & \cdots & \dfrac{\partial T_1}{\partial x_d}\\
    \vdots                             & \ddots & \vdots\\
    \dfrac{\partial T_d}{\partial x_1} & \cdots & \dfrac{\partial T_d}{\partial x_d}
\end{bmatrix} \\
|\nabla T(x)| &= |\det  \nabla T(x)|.
\end{align*}
...
this means that to compute its determinant, you just have to compute the product of the diagonal entries, $\det \nabla T = a_{11} a_{22}$.
:::
