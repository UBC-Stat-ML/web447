...
- ESS per second scaling for popular Monte Carlo methods on Gaussian targets 
...
For many models, HMC is faster than the basic MH algorithm [we saw earlier](../w08_mcmc1/topic04_mh.qmd). 



## Notion of efficiency

We first have to define what we mean by an MCMC algorithm less "costly" than another one.
...
- What you care about: how long does it take to get to a certain [Monte Carlo standard error](../w09_workflow/topic05_mcmc_ess.qmd).
    - i.e.: **seconds per effective sample size** (lower is better). 
:::
