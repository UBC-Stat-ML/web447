...
y_i | \theta &\sim \distPoiss(\theta_1 (\sin(\theta_2 i + \theta_3)))
\end{align*}$$

Some things we look at:

-   The distribution's support for each variable matches the variable's datatype.
-   The datatype of each distribution parameter is adequate (for example, here $\theta_1 (\sin(\theta_2 i + \theta_3)) \ge 0$).
...
will force the mean and variance to be equal, which often does not hold in practice. 
:::
...
Which interval of the intervals is shortest?
