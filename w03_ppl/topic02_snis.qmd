...
-   Let's say you want to approximate $\pr(X = 1 | Y = y)$.
    ...
    -   From [last week's "trick 2"](../w02_discrete_bayes/topic06_decision.qmd), if we take
...
$g$, SNIS requires a **proposal** $q(x)$.
...
-   At each iteration, just as [forward
    ...
    -   We keep track of the probability of the "forced choices",
        -   the product of these probabilities will be the un-normalized
...
-   Call the proposal $M$ times.
    ...
    -   Compute also an un-normalized weight for each of the $M$
        outputs: $$W^\parm = w(X^\parm) = \frac{\gamma(X^\parm)}{q(X^\parm)}.$$
            - Here $w(x)$ is a weighting function.
-   Return the ratio
...
- Art Owen's notes, [available from the author's website](https://artowen.su.domains/mc/Ch-var-is.pdf)
