...
- Recall from [chain rule](../w01_discrete_inference/topic08_chain.qmd): $\gamma(x) = p(x, y) = p(x) p(y | x)$.
- Hence, since the proposal is the prior $q(x) = p(x)$, the weight calculation simplifies to
  $$w(x) = \frac{\gamma(x)}{q(x)} = \frac{p(x) p(y | x)}{p(x)} = p(y | x).$$ 
...
**Proposition:** if $\ex_q[w(X)] < \infty$ and $\ex|w(X) g(X)| < \infty$, then[^1]
