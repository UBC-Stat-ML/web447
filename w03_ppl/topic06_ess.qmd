...
- Estimating the SNIS' error based on running it only once. 
- Underlying theoretical guarantee. 

### Rationale

So far we relied on heuristics to determine the number of Monte Carlo iterations. 
Here we describe a better approach when the test function is an indicator function. 
...
As you can see, this is **pessimistic**, i.e. the actually error seems lower than the estimate 
...
(for example, and indicator function),
$$\text{RMSE} = \sqrt{\ex (G_M - g^*)^2} \le 2 \sqrt{\frac{\ex[W^2]}{M (\ex W)^2}},$$
where $W$ is the un-normalized SNIS weight random variable. 

**Estimating SNIS ESS:** The quantity inside the parenthesis is the effective sample size. It is estimated by computing 
the average weight and the average squared weights: 
$$\begin{align*}
\text{SNIS ESS} = \frac{\ex[W^2]}{M (\ex W)^2} &\approx \text{SNIS ESS estimator} \\
&= \frac{\frac{1}{M} \sum_{m=1}^M (W^\parm)^2}{M (\frac{1}{M} \sum_{m=1}^M W^\parm)^2} \\
