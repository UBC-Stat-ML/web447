...
2. roughly twice[^1] the column `mcse_mean` provides the radius of a 95% confidence interval.
...
```{r}
fit$summary(NULL, c("mean", "sd", "mcse_mean", "ess_bulk"))
```
...
- How can we compute Monte Carlo Standard Errors (MCSE) (i.e., numbers such as in `mcse_mean`)?
...
- Recall the central limit theorem (CLT) for independent and identically distributed (i.i.d.) random variables: 
    - if some random variables $V_i$ are i.i.d. and
...
**Intuition:** because of the dependence between MCMC iterations, the noise of the approximation can be larger compared to the 
  i.i.d. setting.[^5]
...
- We start with the simplest possible scenario:
