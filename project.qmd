...
The course project involves independent work on a topic 
selected from a menu of project themes. 
These projects leave freedom for creativity within constraints designed 
for pedagogical and fair evaluation. 
...
In addition, each team should pick one of the "project themes" listed below, exploring topics building and going beyond what 
we will cover during the course.
If two project proposals are too similar, I reserve the right to assign changes to one of the projects (typically the one with 
the last proposal submission date). 

After selection of a project theme, you should start hunting for a few real-world candidate datasets appropriate for the selected theme. 
Two potential datasets should be listed in the proposal. The final report should analyze at least one real dataset.
...
You should not pick a dataset that has already been analyzed using the same approach as you. 
...
Roughly in increasing order of complexity. During grading I will take into account the complexity of the selected project theme.
For example, if considerable coding is required in the project, it may be possible to use only synthetic data instead of real data 
(consult me and document this request in the proposal).
...
As long as the team submits a reasonable project proposal, I will five full grade (5/5) (along with some feedback). 
Late submission within 2 days will receive 4/5, and 0/5 after the grace period, but I can still provide feedback past the
grace period but no later than April 1st. 
Details of the project can change after submission of the proposals. 
Larger changes are allowed but with my permission, so they should not be discussed at the last minute, 
i.e. before April 1st ideally and certainly before the last lecture.
