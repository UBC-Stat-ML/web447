...
\end{align*}
where we use a subscript $\phi$ on $\ex_\phi[\cdot]$ to emphasize that the 
...
**Definition:** the stochastic gradient estimation problem consists of finding an unbiased estimator for the gradient of
...
- Often (but not always), the function $h$ can be written as:
...
- When $N$ is large "sub-sampling" will trade off:
  - computationally cheaper gradient...
  - ...at the cost of more noise (variance) and hence more SGD iterations. 
