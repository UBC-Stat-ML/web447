...
-   Representative examples of questions to prep for Quiz 2.


### Important note

Make sure to practice on the [similar page for Quiz 1](../w07_quiz1/topic02_practice_questions.qmd)
since quiz 2 will cover that material as well. 



### Libraries needed to run several examples below
...
Make sure to finish these practice problems started in class and left for 
self-study: [revisiting Bayesian GLMs in Stan](../w08_mcmc1/topic06_hands_on.qmd).
...
- For each participant, you have collected two covariates: their age and their number of years of education. 
...
3. On both synthetic and real data, trace plots and ESS look good. 
...
Suppose you see the following output from a Stan MCMC fit object:[^2]

[^2]:  `n_eff` is equivalent to `ess_bulk`, you might see one or other depending on the Stan version you are using.
...
1. Report an 80% confidence interval to capture the Monte Carlo error for the posterior mean of the `prediction` parameter. 
...
3. In the context of Monte Carlo methods, what is more important, a bias of zero (unbiasedness) or consistency? Why?
...
You wish to write a Stan model for logistic regression with normal priors 
...
Recall that the CLT for i.i.d. samples and Markov chains give us the following approximations:
