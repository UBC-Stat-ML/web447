...
The data we analyze records, for each day, the number of text messages exchanged each day during `r length(sms_data)` days. 
...
2. All data points before $C$ use the parameter of cluster 1. 
3. All data points at or after $C$ use the parameter of cluster 2. 

**Mathematical description:**

\begin{align*}
\lambda_i &\sim \distExp(1/100), \text{ for }i \in \{1, 2\}, \\
C &\sim \distUnif\{1, 2, \dots `r length(sms_data)`\}, \\ 
Y_d &\sim \distPoiss(\lambda_C), \text{ for }d \in \{1, 2, \dots, `r length(sms_data)`\}.
\end{align*}
...
forward_only = replicate(1000, forward_posterior(5, 200))
