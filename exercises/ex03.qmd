...
$f:[0,\infty)\to\reals$ and outputs a Monte Carlo estimate of
$\int_0^\infty f(x)\dee x$ using Importance Sampling with reference distribution
$\distExp(1)$ and $n=10000$ i.i.d. samples.
...
$$
3. Test your implementation of `is_exponential` by constructing a 95% confidence
interval for your estimate and checking that it contains the true value.
4. Write a function `is_exponential_10` that is identical to `is_exponential`
except that it uses $\distExp(10)$ as reference. Use it to build another 95%
confidence interval and compare it to the one obtained using `is_exponential`.
