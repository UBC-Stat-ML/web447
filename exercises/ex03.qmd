...
missing in either density function; i.e., if only we have access to $\gamam$ and
$\nu$ such that
$$
...
$$
\begin{aligned}
\ex_\pi[f] &= \int f(x)\pi(x) \dee x \\
&= \int f(x)\frac{\pi(x)}{q(x)}q(x) \dee x \\
&= \ex_\nu\left[ f(x) \frac{\pi(x)}{q(x)} \right] \\
&= \frac{Z_q}{Z_\pi} \ex_\nu\left[f(x) \frac{\gamma(x)}{\nu(x)} \right] \\
\end{aligned}
$$
Using $f\equiv1$ in the above we see that
$$
\ex_\nu\left[\frac{\gamma(x)}{\nu(x)} \right] = \frac{Z_\pi}{Z_q}
$$
Hence
$$
\ex_\pi[f] = \frac{\ex_\nu\left[f(x) \frac{\gamma(x)}{\nu(x)} \right]}{\ex_\nu\left[ \frac{\gamma(x)}{\nu(x)} \right]}.
$$
Recall that the SNIS estimator is
$$
...
$$
The inference engine of our PPL will be SNIS. For this, we will restrict our
PPL to models in which the prior is proper and it is possible to draw i.i.d.
samples from it.
...
**Note**: it is much more numerically robust to use 
`d(dist)(x, log = TRUE)` than to manually compute `log(d(dist)(x))`, especially
...
inference via SNIS using `n` samples. 
