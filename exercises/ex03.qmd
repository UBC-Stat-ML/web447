...
Now, **write** the `observe(value, dist)` function, which should accept two arguments:
    ...
    i. a distribution object `dist` from the package `distr`---for example,
    ```r
    library(distr)
    dist
    ```
    and
    ii. a value $x$ that is in the support of the distribution $d$.

Using these two inputs, your function should add the value of $\log(d(x))$ to the
