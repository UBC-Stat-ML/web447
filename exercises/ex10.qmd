...
$$ \text{KL}(q_\phi || \pi) = \int q_\phi(x) \log \frac{q_\phi(x)}{\pi(x)} \, dx.$$
The *forward KL* is given by
$$ \text{KL}(\pi || q_\phi) = \int \pi(x) \log \frac{\pi(x)}{q_\phi(x)} \, dx.$$
...
1. Prove that 
$$ \nabla_\phi \text{KL}(\pi || q_\phi) = \phi - \mathbb{E}_\pi[X].$$ 
