...
\begin{aligned}
\theta &\sim \mu \\
x_t|\theta &\distiid \nu_\theta, \quad t\in\{1,\dots,T\}
\end{aligned}
$$
where $\mu$ is a prior distribution for the unknown parameter $\theta$, and
$\{x_t\}_{t=1}^T$ is a sequence of observations with conditional distribution
$\nu_\theta$.

1. Write down the posterior distribution of $\theta$ given $\{x_t\}_{t=1}^T$.
2. Suppose now we get an additional data point $x_{T+1}$ with the same conditional
distribution $\nu_\theta$. Show that using the posterior from part 1 as *prior* 
and data equal to just $x_{T+1}$ gives *the same* posterior distribution as
redoing part 1 with the $T+1$ data points.
...
1 to 1000. Show the output of `head` for your dataframe.

5. Visualize the above data using a log-log plot of error versus number of 
observations similar to the one used 
[in this lecture](../w03_ppl/topic05_mc_rates.qmd#empirical-scaling).
