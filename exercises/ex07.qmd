...
Here is a reasonable initialization: 
```r
fit = mod$sample(
  ...
  init = list(list(theta1 = 40, theta2 = 1.0, theta3 = 0.25)),
  ...
)
```
...
as well as a pairplot for each pair of parameters (e.g., the pair plot for $\theta_1$, $\theta_2$ is a 
scatterplot of where each point is an MCMC sample, with the x-axis showing $\theta^{(m)}_1$ 
and the y-axis, $\theta^{(m)}_2$ for MCMC iteration $m$).
