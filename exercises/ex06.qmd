...
$$
\begin{aligned}
\mu_e &\sim \distUnif(0,1) && (\text{diffuse prior})\\
\lambda_e &\sim \distExp(0.001) && (\text{diffuse prior})\\
\mu_i &\sim \distNamed{BetaMP}(0.1, 10) && (\text{incidence is usually low})\\
\lambda_i &\sim \distExp(0.001) && (\text{diffuse prior})\\
p_e|\mu_e,\lambda_e &\sim \distNamed{BetaMP}(\mu_e, \lambda_e) \\
p_i|\mu_i,\lambda_i &\sim \distNamed{BetaMP}(\mu_i, \lambda_i) \\
...
both vaccines. The parameters $(\mu_e,\lambda_e,\mu_i,\lambda_i)$ must be 
...
with 20,000 iterations, and report the estimated posterior probability that 
Moderna is more effective than Pfizer. Is there strong evidence for this claim?
**Note**: it takes a couple of minutes to run 20,000 iterations. Therefore, for
prototyping it may be easier to test your implementation with fewer iterations.
